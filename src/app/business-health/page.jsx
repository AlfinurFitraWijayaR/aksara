...
export default function BusinessHealthPage() {
  ...
  const analyzeBusiness = async () => {
    ...
    try {
      ...
      if (result.data.status === "Sehat" || result.data.score >= 80) {
        color = "text-green-600";
        ringColor = "stroke-green-600";
        bgColor = "bg-green-50 border-green-200";
      } else if (result.data.status === "Waspada" || result.data.score >= 50) {
      ...
      setResult({
        ...
        recommendations: result.data.recommendations?.slice(0, 3) || [],
      });
    } catch (error) {
